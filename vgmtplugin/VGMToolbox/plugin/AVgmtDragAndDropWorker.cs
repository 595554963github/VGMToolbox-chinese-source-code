    ...
    public abstract class AVgmtDragAndDropWorker : BackgroundWorker, IVgmtBackgroundWorker
    {
        ...
        protected void DoTask(IVgmtWorkerStruct pTaskStruct, DoWorkEventArgs e)
        {
                    ...
                    {                       
                        ...
                        catch (Exception ex)
                        {
                            ...
                            this.progressStruct.ErrorMessage = 
                                String.Format(CultureInfo.CurrentCulture, "错误处理<{0}>.错误收到:", path) + ex.Message + Environment.NewLine;
            ...
            catch (Exception ex2)
            {
                this.progressStruct.Clear();
                this.progressStruct.ErrorMessage =
                    String.Format(CultureInfo.CurrentCulture, "执行最终任务时出错.收到错误:{0}{1}", ex2.Message, Environment.NewLine);
                ReportProgress(this.progress, this.progressStruct);
            }
            ...
            foreach (string f in Directory.GetFiles(pPath))
            {
                if (!CancellationPending)
                {
                    ...
                    catch (Exception ex)
                    {
                        this.progressStruct.Clear();
                        this.progressStruct.ErrorMessage = 
                            String.Format(CultureInfo.CurrentCulture, "错误处理<{0}>.收到错误:", f) + ex.Message + Environment.NewLine;
        ...
        protected void ShowOutput(string path, string message, bool isError)
        {
            ...
            if (isError)
            {
                this.progressStruct.ErrorMessage = String.Format("错误:{0}{1}", message, Environment.NewLine);
            }
            else
            {
