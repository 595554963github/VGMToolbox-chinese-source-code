...
namespace _2sftimer
{
...

    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine(String.Format("找不到目录<{0}>", pathTo2sf));
                return;
            }

            if (!File.Exists(pathToSdat))
            {
                Console.WriteLine(String.Format("找不到文件<{0}>", pathToSdat));
                return;
            }

            if (!String.IsNullOrEmpty(smapPath) && !File.Exists(smapPath))
            {
                Console.WriteLine(String.Format("找不到文件<{0}>", smapPath));
                return;
            }
            ...
            // run timing script
            Console.WriteLine(Environment.NewLine + "执行计时脚本");
        ...
        {
            Console.WriteLine("使用方法:2sftimer.exe <mini2sf directory> <sdat path> <filename prefix>");
        }
            ...
            string p2sfFilePath, string pSseqFilePath)
        {
            ...
            try
            {
                ...
                if (File.Exists(midiFilePath))
                {
                    ...
                    else
                    {
                        ...
                        Console.WriteLine("流错误:{0}", Bass.BASS_ErrorGetCode());
                        return false;
                    }
            ...
            {
                Console.WriteLine(String.Format("计时错误{0}:{1}", p2sfFilePath, e.Message));
            }
            finally
            {
                sw.Close();
