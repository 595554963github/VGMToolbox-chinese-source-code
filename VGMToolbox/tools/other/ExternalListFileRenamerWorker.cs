...
namespace VGMToolbox.tools.other
{
    ...
    {
        public const string RENAME_SCRIPT_NAME = "vgmt_重命名脚本.bat";
        public const string UNDO_RENAME_SCRIPT_NAME = "vgmt_undo_重命名脚本.bat";
        ...
        private void RenameFilesFromExternalList(ExternalListFileRenamerStruct inputValues)
        {
                ...
                File.Exists(inputValues.ListFileLocation))
            {
                ...
                {
                    throw new Exception(String.Format("源目录为空."));
                }
                else
                {
                    ...
                    using (FileStream listFs = File.Open(inputValues.ListFileLocation, FileMode.Open, FileAccess.Read, FileShare.Read))
                    {
                        ...
                        {
                            throw new IOException(String.Format("文件名偏移量大于列表文件的大小."));
                        }
                        else
                        {
                            ...
                            if (inputValues.FileNameHasTerminator)
                            {
                                ...
                                {
                                    throw new FormatException(String.Format("无法将文件名终止符字节转换为十六进制:'{0}'", ex1.Message), ex1);
                                }
                            }
                            else // convert static filename size
                            {
                                ...
                                {
                                    throw new FormatException(String.Format("无法将文件名静态大小转换为数字:'{0}'", ex2.Message), ex2);
                                }
                            ...
                            try
                            {
                                ...
                                for (int i = 0; i < sourceFileCount; i++)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        this.progressStruct.GenericMessage = String.Format("警告：重命名<{0}>{1} 时到达列表末尾文件", sourceFile, Environment.NewLine);
                                    ...
                                    {
                                        renameScript.AppendFormat("重命名\"{0}\" \"{1}\" {2}", Path.GetFileName(sourceFile), Path.GetFileName(destinationFile), Environment.NewLine);
                                        undoScript.AppendFormat("重命名\"{0}\" \"{1}\" {2}", Path.GetFileName(destinationFile), Path.GetFileName(sourceFile), Environment.NewLine);
            ...
            {
                throw new IOException(String.Format("目录未找到:<{0}>", inputValues.SourceFolder));
            } // if (Directory.Exists(inputValues.SourceFolder))
        }
        ...
        protected override void OnDoWork(DoWorkEventArgs e)
        {
            ...
            {
                this.progressStruct.Clear();
                this.progressStruct.ErrorMessage = String.Format("重命名错误:{0}{1}", _ex.Message, Environment.NewLine);
