...
namespace VGMToolbox.tools.xsf
{
    class NdsTo2sfWorker : AVgmtDragAndDropWorker, IVgmtBackgroundWorker
    {
        ...
        protected override void DoTaskForFile(string pPath, IVgmtWorkerStruct pMk2sfStruct, DoWorkEventArgs e)
        {
            ...
            this.progressStruct.GenericMessage = String.Format("加工: [{0}]{1}", pPath, Environment.NewLine);
            ...
            if (!filesWereRipped)
            {
                this.progressStruct.GenericMessage = String.Format("{0}未找到可提取的SSEQ/STRM/SWAR数据.", Environment.NewLine);
            }
            else
            {
                this.progressStruct.GenericMessage = String.Format("{0}SSEQ/STRM/SWAR提取的数据.", Environment.NewLine);
            }
            ...
            //---------------
            this.progressStruct.Clear();
            this.progressStruct.GenericMessage = String.Format("搜索SWAVs{0}", Environment.NewLine);
            this.ReportProgress(this.progress, this.progressStruct);
            ...
            //--------------
            this.progressStruct.Clear();
            this.progressStruct.GenericMessage = String.Format("搜索ADXs{0}", Environment.NewLine);
            this.ReportProgress(this.progress, this.progressStruct);
