    ...
    class PspSeqDataFinderWorker : AVgmtDragAndDropWorker, IVgmtBackgroundWorker
    {
        ...
        protected override void DoTaskForFile(string pPath, IVgmtWorkerStruct pPspSeqDataFinderStruct, DoWorkEventArgs e)
        {
            ...
            using (FileStream fs = File.OpenRead(pPath))
            {
                ...
                #region PHD EXTRACT
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = String.Format("{0}提取PHD", Environment.NewLine);
                this.ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
                ...
                while ((offset = ParseFile.GetNextOffset(fs, offset, PPHD_SIGNATURE)) > -1)
                {
                    ...
                    {
                        this.progressStruct.Clear();
                        this.progressStruct.ErrorMessage = String.Format("{0}在偏移量0x处提取PPHD时出错: {1}{2}", offset.ToString("X8"), pphdEx.Message, Environment.NewLine);
                ...
                #region MID EXTRACT
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = String.Format("{0}提取MID", Environment.NewLine);
                this.ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
                        ...
                        if (phdArrayList.Count < midFiles.Count)
                        {
                            if (!midNamingMessageDisplayed)
                            {
                                ...
                                this.progressStruct.ErrorMessage = String.Format(
                                    "警告,无法重新排序MID文件,PHD件比MID件少.{0}", Environment.NewLine);
                ...
                #region PBD EXTRACT
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = String.Format("提取PBD...警告,这可能要花很长时间...{0}", Environment.NewLine);
                this.ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
                ...
                while ((offset = ParseFile.GetNextOffset(fs, offset, Psf.VB_START_BYTES, pspStruct.UseZeroOffsetForPbd, 0x10L, 0x00L, true)) > -1)
                {
                    ...
                    {
                        this.progressStruct.Clear();
                        this.progressStruct.ErrorMessage = String.Format("<{0}>在偏移量0x{1}查找BD错误:{2}{3}", pPath, offset.ToString("X8"), pbdEx.Message, Environment.NewLine);
                        this.ReportProgress(this.progress, this.progressStruct);
                    }
                ...
                for (int i = 0; i < phdArrayList.Count; i++)
                {
                    ...
                    {
                        this.progressStruct.Clear();
                        this.progressStruct.ErrorMessage = String.Format("为 <{0}> 构建 PBD 时出错：{1}指单个VAG，无法确定正确的PBD。跳过...{2}", pPath, phdObject.FileName, Environment.NewLine);
                        this.ReportProgress(this.progress, this.progressStruct);
                    }
                    else
                    {
                        for (int j = 0; j < potentialPbdList.Length; j++)
                        {
                                ...
                                (potentialPbdList[j].length == 0))
                            {
                                ...
                                {
                                    this.progressStruct.Clear();
                                    this.progressStruct.ErrorMessage = String.Format("为<{0}>构建BD时出错:{1}{2}", pPath, ex.Message, Environment.NewLine);
                                    this.ReportProgress(this.progress, this.progressStruct);
                                }
                            }
                        }
            ...
            PphdStruct phdObject)
        {
            ...
            for (int i = 0; i < phdObject.vagLengths.Length; i++)
            {
                ...
                {
                    errorMessage = String.Format("警告,{1}在0x处找到{0}的潜在PBD匹配, " +
                        "但索引会超过数组边界.建议您手动检查是否匹配" +
                        "完成时未找到.{2}", phdObject.FileName,
                        ...
                        if (phdObject.IsSmallSamplePresent)
                        {
                            ...
                            if (matchPercentage >= Psf.MIN_SAMPLE_MATCH_PERCENTAGE)
                            {
                                    ...
                                    ParseFile.CompareSegment(lastLine, 0, Psf2.VB_END_BYTES_2))
                                {
                                    ...
                                    this.progressStruct.Clear();
                                    this.progressStruct.GenericMessage = String.Format("     PHD<{0}>包含小于0x{1}的样本,将使用部分匹配.请务必彻底收听组装好的文件.{2}", ret.FileName, Psf2.MIN_ADPCM_ROW_SIZE.ToString("X8"), Environment.NewLine);
                                    this.ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
