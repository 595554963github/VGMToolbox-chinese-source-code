...
namespace VGMToolbox.tools.xsf
{
    class MkPsf2Worker : BackgroundWorker, IVgmtBackgroundWorker
    {
                ...
                if (uniqueSqFiles != null)
                {
                    ...
                    if (pMkPsf2Struct.CreatePsf2lib)
                    {
                        ...
                        if (uniqueHdFiles.Length > 1)
                        {
                            ...
                            this.progressStruct.ErrorMessage = String.Format("错误：{0}检测到超过1HD/BD对,制作时请只包含1HD/BD对.psf2lib文件", Environment.NewLine);
        ...
        private string[] getUniqueFileNames(string pSourceDirectory, string mask)
        {
            ...
            {
                this.progressStruct.Clear();
                this.progressStruct.ErrorMessage = String.Format("错误:目录{0}未找到.", pSourceDirectory);
            ...
            string makePsf2DestinationPath)
        {
            ...
            {                                
                    ...
                    for (int i = 0; i < totalSequences; i++)
                    {
                        if (validSequenceArray[i])
                        {
                            ...
                            if (isSuccess)
                            {
                                ...
                                this.progressStruct.Clear();
                                this.progressStruct.GenericMessage = String.Format("{0}创建.{1}", outputFile, Environment.NewLine);
                                ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
                        ...
                        {
                            this.progressStruct.Clear();
                            this.progressStruct.GenericMessage = String.Format("警告: {0}.SQ只有一个序列，它是无效的。跳过...", filePrefix) +
                                Environment.NewLine;
                            ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
                        }
                    } // for (int i = 0; i < totalSequences; i++)
                ...
                else // make .psf2lib with HD/BD only
                {
                    ...
                    {
                        this.progressStruct.Clear();
                        this.progressStruct.GenericMessage = String.Format("{0}创建.{1}", pMkPsf2Struct.Psf2LibName, Environment.NewLine);
                        ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
