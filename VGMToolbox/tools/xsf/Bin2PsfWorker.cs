    ...
    {                
                ...
                {                    
                    ...
                    if (pBin2PsfStruct.MakePsfLib)
                    {
                        ...
                        if (vhFiles.Length > 1)
                        {
                            ...
                            this.progressStruct.ErrorMessage = String.Format("{0}错误：检测到超过1个VH/VB对，制作.psflb文件时请仅包含1个VH/VB对", Environment.NewLine);
            ...
            int sepTotalSeqs)
        {
            ...
            if (isSuccess)
            {
                ...
                // report to user
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = String.Format("{0}创建.{1}", Path.GetFileName(builtFilePath), Environment.NewLine);                       
                ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
            }
        }
        ...
        private PsxSequenceType getPsxSequenceType(string filePath)
        {
            ...
            else
            {
                ...
                this.progressStruct.ErrorMessage = String.Format("<{0}错误：无法解析>作为SEQ或SEP类型的序列文件.{1}", filePath, Environment.NewLine);
                ReportProgress(this.progress, this.progressStruct);                                
            }
