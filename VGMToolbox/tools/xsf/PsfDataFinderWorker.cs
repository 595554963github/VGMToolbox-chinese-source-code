    ...
    class PsfDataFinderWorker : AVgmtDragAndDropWorker, IVgmtBackgroundWorker
    {
        ...
        protected override void DoTaskForFile(string pPath, IVgmtWorkerStruct pPsfDataFinderStruct, DoWorkEventArgs e)
        {
            ...
            using (FileStream fs = File.OpenRead(pPath))
            {
                ...
                #region VH EXTRACT
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = String.Format("{0}提取VH", Environment.NewLine);
                this.ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
                ...
                #region SEQ EXTRACT
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = String.Format("{0}提取音序", Environment.NewLine);
                this.ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
                ...
                foreach (Psf.ProbableItemStruct seq in seqFiles)
                {
                            ...
                            if ((vhArrayList.Count < seqFiles.Count))
                            {
                                if (!seqNamingMessageDisplayed)
                                {
                                    ...
                                    this.progressStruct.ErrorMessage = String.Format(
                                        "警告,无法重新排序音序文件,VH文件数量少于音序文件数量.{0}", Environment.NewLine);
                    ...
                    { 
                        this.progressStruct.Clear();
                        this.progressStruct.ErrorMessage = String.Format("发现长度小于0的警告音序,在偏移量0x{1}: {2}{3}", seq.offset.ToString("X8"), seq.length.ToString("X8"), Environment.NewLine);
                        this.ReportProgress(this.progress, this.progressStruct);
                    }
                }
                #endregion
                ...
                #region SEP EXTRACT
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = String.Format("{0}提取SEP", Environment.NewLine);
                this.ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
                ...
                foreach (Psf.ProbableItemStruct sep in sepFiles)
                {
                        ...
                        if (psfStruct.ReorderSeqFiles)
                        {
                            if ((vhArrayList.Count < sepFiles.Count))
                            {
                                if (!sepNamingMessageDisplayed)
                                {
                                    this.progressStruct.Clear();
                                    this.progressStruct.ErrorMessage = String.Format(
                                        "警告,无法重新排序sep文件,VH文件比音序文件少.{0}", Environment.NewLine);
                                    this.ReportProgress(this.progress, this.progressStruct);
                    ...
                    {
                        this.progressStruct.Clear();
                        this.progressStruct.ErrorMessage = String.Format("发现长度小于0的警告SEP,在偏移量0x{1}: {2}{3}", sep.offset.ToString("X8"), sep.length.ToString("X8"), Environment.NewLine);
                        this.ReportProgress(this.progress, this.progressStruct);
                    }
                }
                #endregion
                ...
                #region VB EXTRACT
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = String.Format("{0}提取VB...警告,这需要很长时间...", Environment.NewLine);
                this.ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
                    ...
                    0x10, 0)) > -1)
                {
                    ...
                    if (!CancellationPending)
                    {
                        ...
                        {
                            this.progressStruct.Clear();
                            this.progressStruct.ErrorMessage = String.Format("为<{0}>查找VB错误，在偏移量{1}: {2}{3}", pPath, offset.ToString("X8"), vbEx.Message, Environment.NewLine);
                            this.ReportProgress(this.progress, this.progressStruct);
                        }
                ...
                for (int i = 0; i < vhArrayList.Count; i++)
                {
                    ...
                    {
                        this.progressStruct.Clear();
                        this.progressStruct.ErrorMessage = String.Format("<{0}>错误构建VB: {1}指的是单个VAG,无法确定正确的VB.跳过...{2}", pPath, vhObject.FileName, Environment.NewLine);
                        this.ReportProgress(this.progress, this.progressStruct);
                    }
                    else
                    {
                        for (int j = 0; j < potentialVbList.Length; j++)
                        {
                                ...
                                (potentialVbList[j].length == 0)) 
                            {
                                ...
                                {
                                    this.progressStruct.Clear();
                                    this.progressStruct.ErrorMessage = String.Format("<{0}>错误创建VB:{1}{2}", pPath, ex.Message, Environment.NewLine);
                                    this.ReportProgress(this.progress, this.progressStruct);
                                }
            ...
            VhStruct vhObject, bool RelaxVbEofRestrictions)
        {
            ...
            for (int i = 0; i < vhObject.vbSampleSizes.Length; i++)
            {
                ...
                {
                    errorMessage = String.Format("{0}警告,在0x处找到的潜在 VB 匹配{1}, " +
                        "但索引会超过数组边界.建议您手动检查是否匹配" +
                        "完成时未找到.{2}", vhObject.FileName,
                        ...
                        if (vhObject.IsSmallSamplePresent || vhObject.HasHeaderMismatch)
                        {
                            ...
                            if (matchPercentage >= Psf.MIN_SAMPLE_MATCH_PERCENTAGE)
                            {
                                    ...
                                    ParseFile.CompareSegment(lastLine, 0, Psf.VB_END_BYTES_2))
                                {
                                    ...
                                    {
                                        warningMessageFormat = "     VH<{0}>包含小于0x的样本{1},将使用部分匹配.请务必彻底收听组装好的文件.{2}";
                                    }
                                    else if (vhObject.HasHeaderMismatch)
                                    {
                                        warningMessageFormat = "     VH<{0}>的标头与其内部结构不匹配,将使用部分匹配.请务必彻底收听组装好的文件.{2}";
                                    }
                                    else
                                    {
                                        warningMessageFormat = "     VH<{0}>有错误,将使用部分匹配.请务必彻底收听组装好的文件.{2}";
                                    }
