    ...
    class PsfStubMakerWorker : AVgmtDragAndDropWorker, IVgmtBackgroundWorker
    {
        ...
        protected override void DoTaskForFile(string pPath, IVgmtWorkerStruct pPsfStubMakerStruct, DoWorkEventArgs e)
        {
            if (XsfUtil.IsPsxExe(pPath))
            {
                ...
                //////////////////////////
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = "    -准备工作文件夹." + Environment.NewLine;
                ReportProgress(Constants.ProgressMessageOnly, progressStruct);
                ...
                ////////////////////            
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = "    -执行sigfind/sigfind2." + Environment.NewLine;
                ReportProgress(Constants.ProgressMessageOnly, progressStruct);
                ...
                //////////////////////////////
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = "    -重写驱动程序源和批处理文件." + Environment.NewLine;
                ReportProgress(Constants.ProgressMessageOnly, progressStruct);
                ...
                ////////////////
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = "    -编译存根." + Environment.NewLine;
                ReportProgress(Constants.ProgressMessageOnly, progressStruct);
                ...
                {
                    this.progressStruct.Clear();
                    this.progressStruct.ErrorMessage = "[来自VGMToolbox的错误]未找到编译的输出文件.编译失败，存根文件源代码将被复制到您的源目录进行检查." + Environment.NewLine;
                ...
                //////////////////////
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = "    -删除工作文件夹." + Environment.NewLine;
                ReportProgress(Constants.ProgressMessageOnly, progressStruct);
            ...
            {
                this.progressStruct.Clear();
                this.progressStruct.GenericMessage = String.Format("[{0}]跳过.没有有效的PS-X EXE签名.", pPath) + Environment.NewLine;
                ReportProgress(Constants.ProgressMessageOnly, progressStruct);
            }
        }
        ...
        private string getWarnings(PsfPsyQAddresses addresses)
        {
            ...
            foreach (string label in XsfUtil.GetPsyQFunctionList())            
            {
                ...
                if (String.IsNullOrEmpty(lineValue))
                {
                    warnings.AppendFormat("警告:{0}找不到函数.{1}", label, Environment.NewLine);
                }
            }
