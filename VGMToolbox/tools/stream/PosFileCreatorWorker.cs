    ...
    public class PosFileCreatorWorker : AVgmtDragAndDropWorker, IVgmtBackgroundWorker
    {
                ...
                try
                {
                    ...
                    else if (posStruct.DoLoopStartRiffOffset)
                    {
                        ...
                        catch (IndexOutOfRangeException iorEx)
                        {
                            ...
                            this.progressStruct.GenericMessage = String.Format("错误处理RIFF循环启动<{0}>:{1}{2}", Path.GetFileName(pPath), iorEx.Message, Environment.NewLine);
                    ...
                    if ((loopStartValue != DEFAULT_LOOP_VALUE) && ((int)loopStartValue >= 0))
                    {
                        ...
                        else if (posStruct.DoLoopEndRiffOffset)
                        {
                            ...
                            {
                                this.progressStruct.Clear();
                                this.progressStruct.GenericMessage = String.Format("错误处理RIFF循环结束<{0}>:{1}{2}", Path.GetFileName(pPath), iorEx.Message, Environment.NewLine);
                                ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
                            }
                        }
                        ...
                        if ((loopEndValue != DEFAULT_LOOP_VALUE) && (loopEndValue >= 0))
                        {
                            ...
                            {
                                throw new Exception(String.Format("循环起始值大于或等于循环结束值:{0}", pPath));
                            }
                        }
                        else
                        {
                            throw new IndexOutOfRangeException(String.Format("未找到循环结束值或循环结束小于0：{0}", pPath));
                        }

                    }
                    else
                    {
                        throw new IndexOutOfRangeException(String.Format("未找到循环开始值或循环开始小于 0：{0}", pPath));
                    }
                }
                catch (Exception ex)
                {
                    this.progressStruct.Clear();
                    this.progressStruct.GenericMessage = String.Format("找不到<{0}>的循环点：{1}{2}", Path.GetFileName(pPath), ex.Message, Environment.NewLine);
                    ReportProgress(Constants.ProgressMessageOnly, this.progressStruct);
                }
            }
            #endregion
        ...
        protected override void DoFinalTask(IVgmtWorkerStruct pTaskStruct)
        {
            ...
            foreach (string s in keyList)
            {
                ...
                {
                    // output info
                    outputMessage.AppendFormat("  循环起始:0x{0}{1}", this.WorkingList[s].LoopStart.ToString("X8"), Environment.NewLine);
                    outputMessage.AppendFormat("  循环结束:0x{0}{1}", this.WorkingList[s].LoopEnd.ToString("X8"), Environment.NewLine);
                    ...
                    if (posStruct.PredictLoopShiftForBatch && (predictedShiftCount > (long)(this.WorkingList.Count * .1))) // greater than 10%
                    {
                        ...
                        {
                            outputMessage.AppendFormat("  循环移位（预测）:{0}{1}", loopShift.ToString("D"), Environment.NewLine);
                        }
                        else
                        {
                            outputMessage.AppendFormat("  * 循环移位（预测）:{0} (计算:{1}){2}", loopShift.ToString("D"), this.WorkingList[s].LoopShift.ToString("D"), Environment.NewLine);
                        }
                    }
                    else if (this.WorkingList[s].LoopShift != DEFAULT_LOOP_VALUE)
                    {
                        ...
                        //output info
                        outputMessage.AppendFormat("  循环移位: {0}{1}", loopShift.ToString("D"), Environment.NewLine);
                    }
                    ...
                    using (FileStream posStream = File.Open(posFileName, FileMode.Create, FileAccess.Write))
                    {
                        ...
                        // output info
                        outputMessage.AppendFormat("  POS 文件写入:{0}{1}", Path.GetFileName(posFileName), Environment.NewLine);
                    }
                ...
                else
                {
                    ...
                    // output info
                    outputMessage.AppendFormat("  未找到循环信息.{0}", Environment.NewLine);
                }
