    ...
    class HootXmlBuilderWorker : AVgmtDragAndDropWorker, IVgmtBackgroundWorker
    {
            ...
            DoWorkEventArgs e)
        {
            ...
            else
            {
                            ...
                            if (this.isValidHootFormat(file))
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    this.progressStruct.ErrorMessage =
                                        String.Format(CultureInfo.CurrentCulture, "错误处理 <{0}>.收到错误: ", pPath) + ex.Message + Environment.NewLine;
                ...
                else
                {
                    this.progressStruct.Clear();
                    this.progressStruct.ErrorMessage =
                    String.Format(CultureInfo.CurrentCulture, "错误处理{0}>:找不到.zip文件xml创建的设置名称，请验证您的.zip 文件仅包含以下格式{1}{2}", pPath, SUPPORTED_MULTI_FILE_FORMATS, Environment.NewLine);
                    ReportProgress(progress, this.progressStruct);
                }
            }
        }
        ...
        protected override void DoTaskForFile(string pPath, IVgmtWorkerStruct pHootXmlBuilderStruct, DoWorkEventArgs e)
        {
            ...
            else
            {
                    ...
                    using (FileStream fs = File.OpenRead(pPath))
                    {
                        ...
                        if (dataType != null)
                        {
                            ...
                            if (!String.IsNullOrEmpty(vgmData.GetHootDriver()))
                            {
                                ...
                                hootGame.romlist.archive = "在此处插入档案名称";
                ...
                {
                    this.progressStruct.Clear();
                    this.progressStruct.ErrorMessage = String.Format("错误处理<{0}>.收到错误: ", pPath) + ex.Message;
