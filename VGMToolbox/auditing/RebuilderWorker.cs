    ...
    class RebuilderWorker : BackgroundWorker, IVgmtBackgroundWorker
    {
            ...
            bool hasMultipleExtensions)
        {
            ...
            catch (Exception exception)
            {
                ...
                this.progressStruct.ErrorMessage = "将<" + pSourceName + ">重建到目标时出错:" + exception.Message + Environment.NewLine;
                    ...
                    */
                    ...
                    if ((pAuditingUtil.HasVgmtSizeFlag) && (formatType != null))
                    {
                        ...
                        catch (EndOfStreamException e)
                        {
                            ...
                            this.progressStruct.ErrorMessage = String.Format("错误处理<{0}>作为类型[{1}],回退到完整文件校验和.错误收到:{2}", pFilePath, formatType.Name, e.Message) + Environment.NewLine + Environment.NewLine;
                            ReportProgress(Constants.IgnoreProgress, this.progressStruct);
        ...
        public void rebuildSets(RebuildSetsStruct pRebuildSetsStruct, DoWorkEventArgs e)
        {
            ...
            {
                this.progressStruct.Clear();
                this.progressStruct.ErrorMessage = String.Format("重建集合时出错:{0}{1}", ex.Message, Environment.NewLine);
                ReportProgress(Constants.IgnoreProgress, this.progressStruct);
            }
        }
