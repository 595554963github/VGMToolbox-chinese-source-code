...
namespace VGMToolbox.auditing
{
    ...
    class AuditingUtil
    {
        ...
        {                        
            ...
            haveListText = String.Format("你有{0}of{1}{2}套,", haveGameList.Count,
                datafile.game.Length, datafile.header.description) + Environment.NewLine +
                Environment.NewLine + haveListText;
            missListText = String.Format("您缺少{1}{2}套中的{0}套,", missGameList.Count,
                datafile.game.Length, datafile.header.description) + Environment.NewLine +
            ...
            {
                throw new IOException("创建“有”列表时出错:" + ex.Message);
            }
            ...
            {
                throw new IOException("创建“MISS”列表时出错:" + ex.Message);
            }
            ...
            {
                throw new IOException("创建“FixDAT”文件时出错:" + ex.Message);
            }
            ...
            try
            {
                ...
                if (this.unknownFiles.Count > 0)
                {
                    ...
                    {
                        sw.Write(String.Format("在{0}的现有数据文件中找不到以下文件", datafile.header.description)
            ...
            {
                throw new IOException("创建“未知”列表时出错:" + ex.Message);
            }
        }
        ...
        public static rom BuildRom(BuildRomStruct parameters, out string messages)
        {
            ...
            string messageFormat = "错误处理<{0}>作为类型 [{1}],回退到完整文件校验和.收到错误:{2}{3}";
