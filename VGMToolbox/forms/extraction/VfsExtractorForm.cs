...
namespace VGMToolbox.forms.extraction
{
    public partial class VfsExtractorForm : AVgmtForm
    {
            ...
            : base(pTreeNode)
        {
            ...
            this.btnDoTask.Text = "提取文件";
            this.lblTitle.Text = "虚拟文件系统（VFS）提取器";
            this.tbOutput.Text = "- 从简单的VFS档案中提取文件." + System.Environment.NewLine;
            this.tbOutput.Text += "- 预设的.xml文件应该被放置在<" + PLUGIN_PATH + ">" + System.Environment.NewLine;
            this.tbOutput.Text += "1) 输入设置." + System.Environment.NewLine;
            this.tbOutput.Text += "2) 选择数据文件和头文件并单击'提取文件'" + System.Environment.NewLine;
            this.tbOutput.Text += "          OR" + System.Environment.NewLine;
            this.tbOutput.Text += "将数据文件拖放到应用程序上(拖放不支持头文件)." + System.Environment.NewLine;
        ...
        private bool validateFileCountSection()
        {
            ...
            if (this.rbHeaderSizeOffset.Checked)
            {
                isValid &= this.headerSizeOffsetDescription.IsValid("标头偏移量尺寸");
            }
            else if (this.rbStaticFileCount.Checked)
            {
            ...
            {
                isValid &= this.fileCountOffsetDescription.IsValid("文件计数偏移量");
            }
            else if (this.rbHeaderSizeValue.Checked)
            {
        ...
        private bool validateFileNameSection()
        {
            ...
            if (this.cbFileNameIsPresent.Checked)
            {
                ...
                else
                {
                    ...
                    {
                        isValid &= this.fileNameAbsoluteOffsetOffsetDescription.IsValid("文件名绝对偏移信息");
                    }
                    else if (this.rbFileNameRelativeOffset.Checked)
                    {
                        isValid &= this.fileNameRelativeOffsetOffsetDescription.IsValid("文件名相对偏移量信息");
