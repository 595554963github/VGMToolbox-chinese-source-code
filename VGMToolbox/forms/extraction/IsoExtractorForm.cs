...
namespace VGMToolbox.forms.extraction
{
    public partial class IsoExtractorForm : AVgmtForm
    {
            ...
            : base(pTreeNode)
        {
            ...
            this.lblTitle.Text = "ISO镜像浏览器/提取器";

            this.tbOutput.Text = "浏览和提取光盘图像和档案的内容." + Environment.NewLine;
            this.tbOutput.Text += "- 当前支持的图像类型: .3DS, .CUE/.BIN, .GDI/ISO/BIN, .IMG, .ISO, .MDF, .WUD" + Environment.NewLine;
            this.tbOutput.Text += "- 当前支持的文件系统: 3DS NCSD/CTR, GameCube, Green Book (CD-i), ISO 9660 (PC/PSX/PS2), NullDC GDI (Dreamcast), Opera FS (3DO), Wii Optical Disc, Wii U Optical Disc, XDVDFS (XBOX/XBOX360)" + Environment.NewLine;
            //this.tbOutput.Text += "- 当前支持的档案类型: CRI CPK, Microsoft STFS (XBLA), Nintendo U8 Archives (Wii)" + Environment.NewLine;
            this.tbOutput.Text += "- 当前支持的存档类型: CRI CPK, Nintendo U8 Archives (Wii)" + Environment.NewLine;
            this.tbOutput.Text += String.Format("- WII图像需要'ckey.bin'和/or 'kkey.bin'(对于韩国光盘)在<{0}>目录中.", NintendoWiiOpticalDisc.WII_EXTERNAL_FOLDER) + Environment.NewLine;
            this.tbOutput.Text += String.Format("- WII U图像需要'ckey.bin'在<{0}>目录中,和'disckey.bin'或者'<iso名称>.key'在与源.wud图像文件相同的目录中.", NintendoWiiUOpticalDisc.WIIU_EXTERNAL_FOLDER) + Environment.NewLine;
            this.tbOutput.Text += "- 尚未支持/测试: .CCD, .MDS样式提示表;红皮书音频提取" + Environment.NewLine;
