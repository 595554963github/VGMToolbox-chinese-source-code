...
namespace VGMToolbox.forms.stream
{
    public partial class XmaConvertForm : AVgmtForm
    {
            ...
            : base(pTreeNode)
        {
            ...
            this.tbOutput.Text = String.Format("- 使用外部工具将XMA转换为WAV{0}", Environment.NewLine);
            this.tbOutput.Text += String.Format("- 预设的.xml文件应该放在:<{0}>{1}", PLUGIN_PATH, Environment.NewLine);
            this.tbOutput.Text += String.Format("* 注意：此工具需要Xplorer的“ToWavv.exe”,'xma_test. exe'（xma_parse）由hcs和（可选）xmacode.exe.{0}", Environment.NewLine);
            this.tbOutput.Text += String.Format("  请下载这两个文件并放在以下目录中: <{0}>{1}", Path.GetDirectoryName(XmaConverterWorker.TOWAV_FULL_PATH), Environment.NewLine);
        ...
        private void loadPresetList()
        {
            ...
            foreach (string f in Directory.GetFiles(PLUGIN_PATH, "*.xml", SearchOption.TopDirectoryOnly))
            {
                ...
                {
                    MessageBox.Show(String.Format("加载预设文件时出错<{0}>", Path.GetFileName(f)), "Error");
                }
            }
