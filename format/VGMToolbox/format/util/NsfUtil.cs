...
namespace VGMToolbox.format.util
{
    public class NsfUtil
    {
            ...
            using (FileStream fs = File.OpenRead(pNsfM3uBuilderStruct.Path))
            {
                ...
                if (dataType != null && dataType.Name.Equals("Nsfe"))
                {
                    ...
                    using (StreamWriter sw = File.CreateText(outputFile))
                    {
                        ...
                        sw.WriteLine("#######################################################");
                        sw.WriteLine("#");
                        sw.WriteLine("# 游戏: " + nsfeData.SongName);
                        sw.WriteLine("# 艺术家: " + nsfeData.SongArtist);
                        sw.WriteLine("# 版权: " + nsfeData.SongCopyright);
                        sw.WriteLine("# 抓取者: " + nsfeData.NsfRipper);
                        sw.WriteLine("#");
                        sw.WriteLine("#######################################################");
                        sw.WriteLine();
        ...
        public static void BuildSingleFileM3u(string pPath, Nsfe pNsfeData, string pTrackData, int pIndex, int pTrackIndex)
        {
            ...
            singleSW.WriteLine("#######################################################");
            singleSW.WriteLine("#");
            singleSW.WriteLine("# 游戏: " + pNsfeData.SongName);
            singleSW.WriteLine("# 艺术家: " + pNsfeData.SongArtist);
            singleSW.WriteLine("# 版权: " + pNsfeData.SongCopyright);
            singleSW.WriteLine("# 抓取者: " + pNsfeData.NsfRipper);
            singleSW.WriteLine("# 歌曲: " + NsfUtil.ParseTitle(pNsfeData, pIndex));
            singleSW.WriteLine("#");
            singleSW.WriteLine("#######################################################");
            singleSW.WriteLine();
