...
namespace VGMToolbox.format.util
{
    ...
    public class NezPlugUtil
    {
        ...
        public static NezPlugM3uEntry[] GetNezPlugM3uEntriesFromFile(string pPath)
        {
            ...
            {
                throw new IOException(String.Format("文件未找到:<{0}>{1}", pPath, Environment.NewLine));
            }
            ...
            using (FileStream fs = File.OpenRead(pM3uBuilderStruct.Path))
            {
                    ...
                    (typeof(IEmbeddedTagsFormat).IsAssignableFrom(dataType)))
                {
                    ...
                    using (StreamWriter sw = File.CreateText(outputFile))
                    {
                        sw.WriteLine("#######################################################");
                        sw.WriteLine("#");
                        sw.WriteLine("# 游戏: " + enc.GetString(FileUtil.ReplaceNullByteWithSpace(vgmData.SongName)).Trim());
                        sw.WriteLine("# 艺术家: " + enc.GetString(FileUtil.ReplaceNullByteWithSpace(vgmData.SongArtist)).Trim());
                        sw.WriteLine("# 版权: " + enc.GetString(FileUtil.ReplaceNullByteWithSpace(vgmData.SongCopyright)).Trim());
                        sw.WriteLine("#");
                        sw.WriteLine("#######################################################");
                        sw.WriteLine();
        ...
        public static void BuildSingleFilePlaylist(string pPath, INezPlugPlaylistFormat vgmData, string pTrackData, int pIndex)
        {
            ...
            singleSW.WriteLine("#######################################################");
            singleSW.WriteLine("#");
            singleSW.WriteLine("# 游戏: " + enc.GetString(FileUtil.ReplaceNullByteWithSpace(vgmData.SongName)).Trim());
            singleSW.WriteLine("# 艺术家: " + enc.GetString(FileUtil.ReplaceNullByteWithSpace(vgmData.SongArtist)).Trim());
            singleSW.WriteLine("# 版权: " + enc.GetString(FileUtil.ReplaceNullByteWithSpace(vgmData.SongCopyright)).Trim());
            singleSW.WriteLine("#");
            singleSW.WriteLine("#######################################################");
            singleSW.WriteLine();
        ...
        {
            return "轨道 " + index.ToString().PadLeft(2, '0');
        }
    }
}
