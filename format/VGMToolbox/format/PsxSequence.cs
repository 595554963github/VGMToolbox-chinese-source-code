...
namespace VGMToolbox.format
{
    ...
    public class PsxSequence
    {
        ...
        private PsxSqTimingStruct getTimingInfo(Stream pStream)
        {
            ...
            if (this.forceOppositeFormatType)
            {
                ret.Warnings += "内部版本类型失败，试图强制相反的版本，完成后请验证." + Environment.NewLine;
            }
            ...
            while (pStream.Position < pStream.Length)
            {
                ...
                if (!emptyTimeNext)
                {
                    ...
                    {
                        throw new Exception("节奏未设置，当前滴答不等于零");
                    }
                ...
                if ((currentByte & 0x80) == 0) // data byte, we should be running
                {
                    if (runningCommand == 0)
                    {
                        throw new PsxSeqFormatException(String.Format("清空0x{0}处的运行命令", currentOffset.ToString("X2")));
                    }
                    else
                    {
                        running = true;
        ...
        DONE:       // Marker used for skipping delta ticks at the end of a file.
            ...
            if (loopTimeStack.Count > 0)
            {
                ret.Warnings += "找到不匹配的循环开始标记." + Environment.NewLine;
            ...
            {
                ret.Warnings += "找到不匹配的循环结束标记." + Environment.NewLine;
