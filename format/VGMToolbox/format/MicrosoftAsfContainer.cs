    ...
    public class MicrosoftAsfContainer
    {
        ...
        private void parseHeader(Stream inStream, long currentOffset)
        {
            ...
            for (uint i = 0; i < tempHeader.NumberOfHeaderObjects; i++)
            {
                ...
                {
                    throw new IndexOutOfRangeException(String.Format("解析标头时出错，以0x{0}结尾的对象超过标头大小。{1}", currentOffset.ToString("X"), Environment.NewLine));
                }
            }
        ...
        private AsfDataPacketPayloadParsingObject parseAsfDataPacketPayloadParsingObject(Stream inStream, long currentOffset)
        {
            ...
            {
                throw new FormatException(String.Format("在偏移量0x{0}处处理封包时出错，发现纠错数据.", currentOffset.ToString("X8")));
            }
        ...
        private AsfDataPacketPayloadObject parseAsfDataPacketPayloadObject(Stream inStream, long currentOffset, AsfDataPacketPayloadParsingObject payloadParsingObject, MpegStream.DemuxOptionsStruct demuxOptions)
        {
            ...
            {
                throw new FormatException(String.Format("E在偏移量0x{0}处处理封包有效载荷时出错，不支持压缩有效载荷.", currentOffset.ToString("X8")));
            }
        ...
        protected virtual void DoFinalTasks(MpegStream.DemuxOptionsStruct demuxOptions)
        {
            ...
            foreach (uint key in this.streamWriterDictionary.Keys)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        throw new Exception(String.Format("文件的异常构建标头:{0}{1}{2}", this.streamWriterDictionary[key].Name, ex.Message, Environment.NewLine));
                    }
                    else
                    {
                        throw new Exception(String.Format("文件的异常构建标头:{0}{1}{2}", "未知", ex.Message, Environment.NewLine));
                    }
                }
                finally
                {
        ...
        public virtual void DemultiplexStreams(MpegStream.DemuxOptionsStruct demuxOptions)
        {
            ...
            try
            {
                using (FileStream fs = File.OpenRead(this.FilePath))
                {
                            ...
                            else if (checkGuid.Equals(MicrosoftAsfContainer.ASF_Data_Object))
                            {
                                ...
                                for (ulong i = 0; i < this.DataObject.TotalDataPackets; i++)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        if (packetSize < 0)
                                        {
                                            throw new Exception(String.Format("在偏移量0x{0}处解析封包时出错.", packetOffset.ToString("X8")));
                                        }
                                        else
                                        {
                                    ...
                                    {
                                        throw new Exception(String.Format("在偏移量0x{0}处解析封包时出错：{1}.{2}", packetOffset.ToString("X8"), ex.Message, Environment.NewLine));
                                    }
                                }
                            }
                    ...
                    {
                        throw new Exception(String.Format("ASF/WMV标头未找到.{0}", Environment.NewLine));
                    }
            ...
            {
                throw new Exception(String.Format("偏移量0x{0}处的异常处理块：{1}{2}", currentOffset.ToString("X"), ex.Message, Environment.NewLine));
            }
            finally
            {
                // clean up
