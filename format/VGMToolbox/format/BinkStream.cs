            ...
            MpegStream.DemuxOptionsStruct demuxOptions)
        {
            ...
            foreach (uint key in streamWriters.Keys)
            {
                try
                {
                    if (demuxOptions.AddHeader)
                    {
                        ...
                        else if (streamWriters[key].Name.EndsWith(BinkStream.DefaultFileExtensionAudioSplit))
                        {
                            ...
                            {
                                throw new FormatException("拆分音频流不支持的Bink类型.");
                            }
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        throw new Exception(String.Format("文件的异常构建标头:{0}{1}{2}", streamWriters[key].Name, ex.Message, Environment.NewLine));
                    }
                    else
                    {
            ...
            try
            {
                using (FileStream fs = File.OpenRead(this.FilePath))
                {
                    ...
                    for (uint frameId = 0; frameId < this.FrameCount; frameId++)
                    {
                        ...
                        {
                            throw new Exception(String.Format("异常处理帧0x{0}在偏移量0x{1}：{2}{3", frameId.ToString("X"), this.FrameOffsetList[frameId].FrameOffset.ToString("X"), fex.Message, Environment.NewLine));
                        }
                    } // for (uint frameId = 0; frameId < this.FrameCount; frameId++)
