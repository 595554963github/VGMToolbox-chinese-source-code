...
namespace VGMToolbox.format
{
    ...
    public class CriUtfTable
    {
        ...
        public const string TEMP_UTF_TABLE_FILE = "UTF.BIN";
        ...
        public void Initialize(FileStream fs, long offset)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    throw new FormatException(String.Format("@UTF在偏移量<0x{0}>处找不到签名.", offset.ToString("X8")));
                }
            }
            catch (Exception ex)
            {
        ...
        private void checkEncryption(FileStream fs, bool useIncomingKeys = false, Dictionary<string, byte> incomingLcgEncryptionKeys = null)
        {
            ...
            else
            {
                ...
                {
                    throw new FormatException(String.Format("无法在偏移量处解密UTF表：0x{0}", this.BaseOffset.ToString("X8")));
                }
                else
                {
                        ...
                        ((field.Type & COLUMN_STORAGE_MASK) == COLUMN_STORAGE_CONSTANT2))
                    {
                        ...
                        switch (field.Type & COLUMN_TYPE_MASK)
                        {
                            ...
                            case COLUMN_TYPE_1BYTE:
                                ...
                                break;
                            default:
                                throw new FormatException(String.Format("偏移处的未知列类型：0x{0", currentOffset.ToString("X8")));
