...
namespace VGMToolbox.format
{
    public class NintendoWad
    {
        ...
        public const string FORMAT_DESCRIPTION_STRING = "任天堂WAD";
        public const string EXTRACTION_FOLDER = "VGMT_WAD提取";
        public const string EXTRACTION_FOLDER_NONCONTENT = "_non-内容";
        ...
        public NintendoWad(string sourceFile)
        {
            ...
            if (NintendoWad.IsWadFile(sourceFile))
            {
                ...
                using (FileStream fs = File.OpenRead(sourceFile))
                {
                    ...
                    // offsets
                    this.CertificateChainOffset = (uint)this.PadValue(this.HeaderSize, "证书链偏移");
                    this.TicketOffset = (uint)this.PadValue(this.CertificateChainOffset + this.CertificateChainSize, "票据偏移量");
                    this.TitleMetaDataOffset = (uint)this.PadValue(this.TicketOffset + this.TicketSize, "标题元数据偏移量");
                    this.DataOffset = (uint)this.PadValue(this.TitleMetaDataOffset + this.TitleMetaDataSize, "数据偏移量");
                    this.FooterOffset = (uint)this.PadValue(this.DataOffset + this.DataSize, "页脚偏移量");
        ...
        private void ParseTmdContentEntries(FileStream fs)
        {
            ...
            for (ushort i = 0; i < this.NumberOfContents; i++)
            {
                ...
                nextOffset = this.PadValue((contentEntry.ContentOffset + contentEntry.ContentSize),
                    String.Format("内容ID的下一个偏移量：0x{0}", contentEntry.ContentId.ToString("X8")));
            }
        ...
        private ulong PadValue(ulong value, string textDescription)
        {
            ...
            {
                throw new Exception(String.Format("\"{0}的错误填充值.\"", textDescription));
            }
            else
            {
