...
namespace VGMToolbox.format.brsar
{
    class Brsar //: IFormat
    {
        ...
        private void initializeTagHash()
        {
            ...
            tagHash.Add("文件大小", HEX_PREFIX + BitConverter.ToUInt32(this.StdHeaderFileSize, 0).ToString("X4"));
            tagHash.Add("标头大小", HEX_PREFIX + BitConverter.ToUInt16(this.StdHeaderHeaderSize, 0).ToString("X2"));
            tagHash.Add("节数", HEX_PREFIX + BitConverter.ToUInt16(this.StdHeaderNumberOfSections, 0).ToString("X2"));

            tagHash.Add("SYMB偏移量,长度",
            ...
            if (SymbSection != null)
            {
                tagHash.Add("SYMB-截面尺寸", HEX_PREFIX + BitConverter.ToUInt32(SymbSection.StdHeaderSectionSize, 0).ToString("X4"));
            }
