            ...
            try
            {
                using (FileStream fs = File.OpenRead(this.FilePath))
                {
                    ...
                    if (mvhdOffset >= 0)
                    {
                        if (currentOffset >= 0)
                        {
                            ...
                            while (currentOffset < fileSize)
                            {
                                ...
                                if (demuxOptions.ExtractAudio && this.IsThisAnAudioBlock(chunkId))
                                {
                                    ...
                                    this.writeChunkToStream(fullChunk, "音频", streamWriters, this.FileExtensionAudio);
                                }
                                ...
                                {
                                    fullChunk = ParseFile.ParseSimpleOffset(fs, currentOffset, (int)chunkSize);
                                    this.writeChunkToStream(fullChunk, "视频", streamWriters, this.FileExtensionVideo);
                                }
                                ...
                                {
                                    fullChunk = ParseFile.ParseSimpleOffset(fs, currentOffset, (int)chunkSize);
                                    this.writeChunkToStream(fullChunk, "未知", streamWriters, ".bin");
                                }
                        ...
                        {
                            throw new Exception(String.Format("找不到MVHD、SCHl或SHEN标头.{0}", Environment.NewLine));
                        }
                    }
                    else
                    {
                        throw new Exception(String.Format("找不到MVHD标头.{0}", Environment.NewLine));
                    } // if (mvhdOffset >= 0)
                }
            }
            catch (Exception ex)
