...
namespace VGMToolbox.format.iso
{
    ...
    public class NintendoWiiUOpticalDisc
    {
        ...
        public static string FORMAT_DESCRIPTION_STRING = "WiiU 加密的";
        public static string FORMAT_DESCRIPTION_STRING_DECRYPTED = "WiiU 已解密";
        ...
        public static byte[] GetKeyFromFile(string path)
        {
            ...
            if (!File.Exists(path))
            {
                throw new FileNotFoundException(String.Format("未找到密钥:{0}.{1}", path, Environment.NewLine));
            }
            else
            {
        ...
        private void InitializePartions(FileStream isoStream)
        {
            ...
            {
                throw new FormatException("未找到分区TOC签名字节.");
            }
        ...
        private void ParsePartitionFileTables(FileStream isoStream)
        {
            ...
            for (uint i = 0; i < this.Partitions.Length; i++)
            {
                    ...
                    this.TitleKeyHash.ContainsKey(partitionNameHashKey))
                {
                    ...
                    {
                        throw new FormatException(String.Format("找不到分区的文件表签名：{0}.", this.Partitions[i].PartitionName));
                    }
                ...
                {
                    MessageBox.Show(String.Format("警告，发现不匹配的分区：{0}，如果这是原始（良好）光盘映像，则可能是VGMToolbox的bug.", this.Partitions[i].PartitionName), "Warning - Unmatched Partition Found");
                }
        ...
        public static string GetPartitionTextDescription(NintendoWiiUPartition partition)
        {
            ...
            sb.AppendLine("分区:" + partition.PartitionName);

            sb.AppendFormat("密钥:{0}{1}", BitConverter.ToString(partition.PartitionKey).Replace("-", ""), Environment.NewLine);
            sb.AppendFormat("IV:{0}{1}", BitConverter.ToString(partition.IV).Replace("-", ""), Environment.NewLine);
            sb.AppendFormat("偏移量:0x{0}{1}", partition.PartitionOffset.ToString("X2"), Environment.NewLine);
            sb.AppendLine();
            ...
            {
                sb.AppendFormat("集群0x{0}{1}", i.ToString("X2"), Environment.NewLine);
                sb.AppendFormat("偏移量0x{0}{1}", partition.PartitionClusters[i].StartOffset.ToString("X2"), Environment.NewLine);
                sb.AppendFormat("尺寸:0x{0}{1}", partition.PartitionClusters[i].Size.ToString("X2"), Environment.NewLine);
            }
    ...
    public class NintendoWiiUOpticalDiscVolume : IVolume
    {
        ...
        public int CompareTo(object obj)
        {
            ...
            throw new ArgumentException("对象不是任天堂WiiU光盘卷");
        }
    ...
    public class NintendoWiiUOpticalDiscFileStructure : IFileStructure
    {
        ...
        public int CompareTo(object obj)
        {
            ...
            throw new ArgumentException("对象不是任天堂WiiU光盘卷文件结构");
        }
    ...
    public class NintendoWiiUOpticalDiscDirectoryStructure : IDirectoryStructure
    {
        ...
        public int CompareTo(object obj)
        {
            ...
            throw new ArgumentException("对象不是任天堂WiiU光盘卷目录结构");
        }
            ...
            using (FileStream outStream = File.Open(destinationPath, FileMode.Create, FileAccess.Write))
            {
                while (size > 0)
                {
                    ...
                    if (!ParseFile.CompareSegment(blockSha1, 0, h0))
                    {
                        ...
                        {
                            MessageBox.Show(String.Format("警告：“{0}”，提取期间偏移量0x{1}处的SHA1验证失败.{2}",
                                Path.GetFileName(destinationPath), readOffset, Environment.NewLine), "警告-SHA1验证失败");
                        }
                    }
