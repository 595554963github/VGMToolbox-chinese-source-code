...
namespace VGMToolbox.format
{
    ...
    public class Nintendo3dsCtr
    {
        ...
        public static string FORMAT_DESCRIPTION_STRING = "3DS CTR 加密的"; // currently unsupported
        public static string FORMAT_DESCRIPTION_STRING_DECRYPTED = "3DS CTR 已解密";
    ...
    public class Nintendo3dsCtrFile : IFileStructure
    {
        ...
        public int CompareTo(object obj)
        {
            ...
            throw new ArgumentException("对象不是一个Nintendo3dsCtrFile");
        }
        ...
        public void Extract(ref Dictionary<string, FileStream> streamCache, string destinationFolder, bool extractAsRaw)
        {
            ...
            if (this.FileSystem == Nintendo3dsCtr.FileSystemType.ExeFS)
            {
                ...
                if (!ParseFile.CompareSegment(outputHash, 0, this.Sha256Hash))
                {
                    ...
                    {
                        MessageBox.Show(String.Format("警告：“{0}”，提取期间在ExeFS中偏移量0x{1}处的SHA256验证失败。{2}",
                            Path.GetFileName(destinationFile), this.Offset, Environment.NewLine), "警告-SHA256故障");
                    }
                }
            }
            //---------------
    ...
    public class Nintendo3dsCtrDirectory : IDirectoryStructure
    {
        ...
        private void ParseIvfcHeader(FileStream fs, long offset)
        {
            ...
            {
                throw new FormatException(String.Format("未找到IVFC字节."));
            }
        ...
        private void ValidateIvfcContainer(FileStream fs, long offset) // thanks to neimod's ctrtool
        {
            ...
            for (ulong i = 0; i < 3; i++)
            {
                ...
                {
                    throw new Exception("错误，IVFC块不匹配.");
                }
                ...
                if (badHashCount > 0)
                {
                    hashFailures.AppendFormat("级别{0}中的IVFC哈希失败。好块：{1}坏块：{2}{3}", i + 1, goodHashCount.ToString(), badHashCount.ToString(), Environment.NewLine);
                }
                else
                {
                    hashFailures.AppendFormat("在级别{0}检查中未发现IVFC，哈希失败。{1}", i + 1, Environment.NewLine);
                }
            } // for (ulong i = 0;...
            ...
            if (hashFailures.Length > 0)
            {
                ...
                MessageBox.Show(hashFailures.ToString(), "哈希验证结果");
            }
        }
        ...
        public void InitializeRomFileSystem(FileStream isoStream, long offset)
        {
            ...
            DialogResult m = MessageBox.Show("根据内部哈希/校验和验证分区?", "验证完整性?", MessageBoxButtons.YesNo);
        ...
        public int CompareTo(object obj)
        {
            ...
            throw new ArgumentException("对象不是一个Nintendo3dsCtrDirectory");
        }
